...
class GF_MESSAGEWAY_WP_SMS
{
	...
	public static function button($field_groups)
	{
		foreach ($field_groups as &$group) {
			if ($group["name"] == "gravity_sms_fields") {
				$group["fields"][] = array("class" => "button", "value" => __('WP SMS', 'GF_SMS'), "data-type" => "sms_subscribtion",);
				break;
			}
		}
		return $field_groups;
	...
	public static function js()
	{
        ...
        <script type='text/javascript'>
            ...
            function wp_sms_panelstd(type) {
                ...
                jQuery('#field_wp_sms_name').val(field["field_wp_sms_name"]);
                jQuery('#field_wp_sms_mobile').val(field["field_wp_sms_mobile"]);

            }

            jQuery(document).bind("gform_load_field_settings", function (event, field, form) {
                wp_sms_panelstd(field.wp_sms_subs);
                jQuery('input[name="wp_sms_subs"]').on("click", function () {
                    wp_sms_panelstd(jQuery('input[name="wp_sms_subs"]:checked').val());
                });
            });
	...
	{
		return array("name" => __('Name field', 'GF_SMS'), "mobile" => __('Mobile number field', 'GF_SMS'), "country_code_dynamic" => '',);
	}
			...
			if ($is_form_editor || $type == 'select') {
				if (!is_admin() && $field->enableEnhancedUI) {
					...
					foreach ($form['fields'] as $field_val) {
						...
						if ($field_val->enableEnhancedUI && in_array($input_type, array('select', 'multiselect', 'sms_subscribtion'))) {
		...
		foreach ((array)$wp_sms_fileds as $field) {
			...
			if (!empty($process['status']) && $process['status'] == 'success-1') {
				if (get_option('wp_subscribes_send_sms') && rgar($field, 'wp_sms_welcome_msg')) {
					...
					GF_MESSAGEWAY_Form_Send::Send($mobile, $final_message, $from = '', $form['id'], '', '');
				}
			}
		}
		return $entry;
		...
		if (empty($mobile_exist) || $type != 'subscribe') {
			if ($type == 'subscribe') {
				...
				foreach ((array)$groups as $group) {
					$insert = $wpdb->insert("{$table_prefix}sms_subscribes", array('date' => date('Y-m-d H:i:s', current_time('timestamp', 0)), 'name' => $name, 'mobile' => $mobile, 'status' => '1', 'group_ID' => $group));
					if ($insert) {
						return array('status' => 'success-1', 'message' => __('Subscription to the SMS newsletter has been successfully done.', 'GF_SMS'));
					} else {
						return array('status' => 'failed-1', 'message' => __('Subscription to the SMS newsletter failed.', 'GF_SMS'));
					}
				}
				...
				if (!empty($mobile_exist)) {
					...
					if (!empty($delete)) {
						return array('status' => 'success-2', 'message' => __('Unsubscription to the SMS newsletter has been successfully done.', 'GF_SMS'));
					} else {
						return array('status' => 'failed-2', 'message' => __('Unsubscription to the SMS newsletter failed.', 'GF_SMS'));
					}
				} else {
					return array('status' => 'not-sub', 'message' => __('The entered number was not subscribed to the SMS newsletter and the unsubscription did not take place.', 'GF_SMS'));
				}
			}
		} else {
			return array('status' => 'repeat', 'message' => __('The entered mobile number has previously been used in the newsletter.', 'GF_SMS'));
		}
	}
}