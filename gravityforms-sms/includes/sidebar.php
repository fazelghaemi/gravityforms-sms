...
class GF_MESSAGEWAY_Entries_Sidebar
{
	...
	public static function get_phone_numbers($form_id, $entry)
	{
		if (gform_get_meta($entry['id'], 'client_mobile_numbers')) {
			...
			$feeds = GF_MESSAGEWAY_SQL::get_feed_via_formid($form_id, true);
			foreach ((array)$feeds as $feed) {
				...
				if (!empty($feed['meta']['customer_field_clientnum'])) {
					...
					$client1 = !empty($mobile) ? GF_MESSAGEWAY_Form_Send::change_mobile($mobile, $code) : '';
				}
				if (!empty($feed['meta']['to_c'])) {
					$client2 = GF_MESSAGEWAY_Form_Send::change_mobile($feed['meta']['to_c'], $static_code);
				}
	...
	public static function client_number_edit($form_id, $entry)
	{
		...
		if (rgpost("save") && (RGForms::post("screen_mode") == "edit" || RGForms::post("action") != "update")) { ?>
            <label for="smspanel_client_edit"><?php _e('Mobile number: ', 'GF_SMS') ?></label>
            <input type="text" name="smspanel_client_edit" id="smspanel_client_edit"
	...
	{
		if (!rgpost("smspanel_client_edit")) {
			return;
		}
		...
		if ($client_nums != rgpost("smspanel_client_edit")) {
			global $current_user;
			$user_id = 0;
			...
			$mobile = rgpost("smspanel_client_edit") ? sanitize_text_field(rgpost("smspanel_client_edit")) : ' ';
	...
	public static function add_meta_boxes($meta_boxes, $entry, $form)
	{
		...
		foreach ($meta_boxes as $key => $val) {
			...
			if ($key == 'submitdiv') {
				$new_meta_boxes['send_sms'] = array('title' => __(' Send SMS ', 'GF_SMS'), 'callback' => array(__CLASS__, 'send_sms_sidebar'), 'context' => 'side',);
			}
		}
	...
	public static function send_sms_sidebar($arg_1, $arg_2)
	{
		...
		if (rgpost("smspanel_send") && rgpost("gf_smspanel_sideber") && wp_verify_nonce(rgpost("gf_smspanel_sideber"), "send")) {
			$from = sanitize_text_field(rgpost('smspanel_from'));
			$from_db = get_option("gf_sms_last_sender");
			if ($from and $from_db != $from) {
				update_option("gf_sms_last_sender", $from);
			}
			$to = sanitize_text_field(rgpost('smspanel_client'));
			$msg = GFCommon::replace_variables(wp_kses(rgpost('smspanel_text'), array('br' => array())), $form, $entry);
			...
			if ($to) {
				...
				if ($result == 'OK') {
					GF_MESSAGEWAY_SQL::save_sms_sent($form['id'], $entry['id'], $from, $to, $msg, '');
        ...
        <h3 style="border-bottom:1px solid #ededed;padding:7px 14px"><?php echo __(' Send SMS ', 'GF_SMS') . GF_MESSAGEWAY::show_credit($settings["cr"], false); ?></h3>
        <div class="inside">
	<?php } ?>

		<?php if ($is_OK) { ?>

		<?php wp_nonce_field("send", "gf_smspanel_sideber") ?>

        <div id="minor-publishing" style="padding:10px;">

            <label for="smspanel_client"><?php _e('Reciever numbers : ', 'GF_SMS'); ?></label>
            <input type="text" name="smspanel_client"
            ...
            <label for="smspanel_text"><?php _e('Message : ', 'GF_SMS'); ?></label>
            <select style="width:100%" id="smspanel_text_variable_select"
                    onchange="InsertMegeTag_SMS( 'smspanel_text' , 'variable_select', jQuery(this).val());">
				<?php $form_meta = RGFormsModel::get_form_meta($form['id']);
				echo GF_MESSAGEWAY_Configurations::get_form_fields_merge($form_meta); ?>
            </select>

            <textarea id="smspanel_text" class="input-text"
                      style="width: 100%; height: 100px; padding:5px;" name="smspanel_text"></textarea>
        </div>
        ...
        <div id="major-publishing-actions">
            ...
            <div id="delete-action" style="width:70%">
                ...
                <select id="smspanel_from" name="smspanel_from" style="width:100%">
            ...
            <div id="publishing-action" style="width:25%">
                <input class="button button-large button-primary" type="submit" name="smspanel_send"
