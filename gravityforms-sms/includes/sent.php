...
class GF_SMS_Sent_List_Table extends WP_List_Table
{
	...
	{
		global $status, $page;
		parent::__construct(array('singular' => __('SMS', 'GF_SMS'), 'plural' => __('SMS', 'GF_SMS'), 'ajax' => false));
	}
	...
	function column_default($item, $column_name)
	{
		...
		switch ($column_name) {
			...
			case 'date':
				$date = date_i18n('Y-m-d H:i:s', strtotime($item[$column_name]));
				return $date;
			case 'sender':
			case 'reciever':
	...
	{
		$sortable_columns = array('date' => array('date', false), 'form_id' => array('form_id', false), 'entry_id' => array('entry_id', false), 'sender' => array('sender', false), 'reciever' => array('reciever', false), 'message' => array('message', false));
		return $sortable_columns;
	}

	function get_columns()
	{
		$columns = array('cb' => '<input type="checkbox" />', 'date' => __('Date', 'GF_SMS'), 'form_id' => __('Form ID', 'GF_SMS'), 'entry_id' => __('Entry ID', 'GF_SMS'), 'sender' => __('From', 'GF_SMS'), 'reciever' => __('Recievers', 'GF_SMS'), 'message' => __('Message', 'GF_SMS'));
		return $columns;
	}
	...
	function column_date($item)
	{
		...
		$actions = array('delete' => sprintf('<a href="?page=%s&view=sent&action=%s&item=%s&_wpnonce=%s">%s</a>', esc_attr($_REQUEST['page']), 'delete', absint($item['id']), $delete_nonce, __('Delete', 'GF_SMS')),);
	...
	{
		$actions = array('bulk_delete' => __('Delete', 'GF_SMS'));
		return $actions;
	}
	...
	{
		global $wpdb;
		$sent_table_name = GF_MESSAGEWAY_SQL::sent_table();
	...
	public function process_bulk_action()
	{
		...
		if ('delete' === $action) {
			if (!empty($_REQUEST) && !wp_verify_nonce(sanitize_text_field($_REQUEST['_wpnonce']), 'gf_delete_sms')) {
				die(__('Permission denaied!', 'GF_SMS'));
			}
